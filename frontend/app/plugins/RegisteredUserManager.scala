...
return
  ...
  def clientSecret = Play.current.configuration.getString("google.clientsecret").getOrElse{throw new RuntimeException("'google.clientsecret' configuration required")}

  def clientId = Play.current.configuration.getString("google.clientid").getOrElse{throw new RuntimeException("'google.clientid' configuration required")}
